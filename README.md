...
![RA Engine: a chunked voxel valley at dusk, with god rays through the cloud layer](docs/images/world.png)

<details>
<summary><b>More screenshots</b> (lessons, dialogue, narration, night sky, combat)</summary>

<br>

**Jericho, a lesson authored entirely in JSON.** Quest objectives track live in the HUD; the NPC, the terrain build ops and the objective list all come from `assets/lessons/jericho.json`.

![The Jericho lesson: an NPC named Joshua on a grass plain, with a three-item objective checklist in the HUD](docs/images/lesson-jericho.png)

**Branching dialogue and narration**, both teacher-editable JSON.

![Dialogue with an NPC, with a narration banner above and a dialogue box below](docs/images/dialogue.png)

![The Garden of Eden lesson opening with a narration banner over a wooded valley](docs/images/narration.png)

**Time of day and weather** drive the sky, the lighting and the synthesized ambience together.

![Night sky with a moon, drifting clouds and stars over the voxel world](docs/images/night-sky.png)

**Combat** against archetype enemies with line-of-sight pursuit and ledge climbing.

![Two enemies on a grass plain, seen over the sling weapon indicator](docs/images/combat.png)
...
[MIT](LICENSE). Block textures and audio are generated by the game at runtime rather than
shipped as assets. The three rigged glTF base models under `assets/models/base/` are original
work, authored in Blockbench in a separate repository.
